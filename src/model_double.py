...
class MultiModal(nn.Module):
    def __init__(self, args):
        ...
        self.bert_text = TextUniBertForMaskedLM.from_pretrained(model_path, config=uni_bert_cfg)
        ...
        # self.linear = nn.Linear(bert_output_size, bert_output_size)        
        ...
        if 'mlm' in self.task:
            ...
            self.conv = nn.Conv2d(in_channels=args.bert_seq_length+32+1, out_channels=args.bert_seq_length-1, kernel_size=(1, 1), stride=1, dilation=1, bias=False)
            self.clss = BertOnlyMLMHead(uni_bert_cfg)
        if 'itm' in self.task:
            self.sv = ShuffleVideo()
            self.newfc_itm = torch.nn.Linear(uni_bert_cfg.hidden_size, 1) 
        if 'clip' in self.task:
    ...
    def forward(self, inputs):
        ...
        Text_features, lm_prediction_scores = self.bert_text(text_input_ids, text_mask, is_usemask=False, return_mlm=return_mlm)
        
        tfidffea = inputs['tfidf'].to(self.device).unsqueeze(1)
        tfidffea = self.tfs(tfidffea)
        video_feature = torch.cat((video_feature, tfidffea), dim=1)
        video_feature = self.trans0(video_feature)
        features = torch.cat((Text_features, video_feature), dim=1)  
        ...
        if 'clip' in self.task:
            ...
            image_features = torch.mean(video_feature, dim=1, keepdim=False)
            # normalized features
            ...
            loss_img = nn.CrossEntropyLoss().to(self.device)
            loss_txt = nn.CrossEntropyLoss().to(self.device)
            ground_truth = torch.arange(inputs['frame_input'].shape[0], dtype=torch.long, device=self.device)
    ...
    def acti(self, inputs):
        ...
        features = self.multiple_dropout(features)
        embedding = self.embedding(features)
        return embedding
...
class TextUniBertForMaskedLM(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.bert = TextUniBert(config)
        self.cls = BertOnlyMLMHead(config)
        self.pooler = BertPooler(config)

    # Copied from transformers.models.bert.modeling_bert.BertModel.forward
    def forward(self, text_input_ids, text_mask, is_usemask=False, return_mlm=False):

        encoder_outputs = self.bert(text_input_ids, text_mask, is_usemask=is_usemask)
        if return_mlm:
            return encoder_outputs, self.cls(encoder_outputs)[:, 1:, :]
        else:
            return encoder_outputs, None   
            
class TextUniBert(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config
    ...
    # Copied from transformers.models.bert.modeling_bert.BertModel.forward
    def forward(self,  text_input_ids, text_mask, is_usemask=False):
