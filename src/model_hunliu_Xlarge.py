...
class MultiModal(nn.Module):
    def __init__(self, args):
        ...
        self.convvidtxt2 = nn.Conv2d(in_channels=33, out_channels=args.bert_seq_length+33, kernel_size=(1, 1), stride=1, dilation=1, bias=False)
        self.convup3 = nn.Conv2d(in_channels=32, out_channels=33, kernel_size=(1, 1), stride=1, dilation=1, bias=False)
        self.convup4 = nn.Conv2d(in_channels=args.bert_seq_length+33, out_channels=args.bert_seq_length+32*2+1*2, kernel_size=(1, 1), stride=1, dilation=1, bias=False)
        ...
        if 'mlm' in self.task:
            ...
            self.conv = nn.Conv2d(in_channels=args.bert_seq_length+32*2+1*2, out_channels=args.bert_seq_length-1, kernel_size=(1, 1), stride=1, dilation=1, bias=False)
    ...
    def forward(self, inputs):
        ...
        video_feature1 = video_feature + self.convtdvid1(tfidffea.unsqueeze(2)).squeeze(2)
        video_feature1 = self.convvidtxt1(video_feature1.unsqueeze(2)).squeeze(2)
        video_feature2 = torch.cat((video_feature, tfidffea), dim=1)

        Text_features, lm_prediction_scores = self.bert_text(video_feature2, video_feature1, text_input_ids, text_mask, is_usemask=False, return_mlm=return_mlm)
        
        video_feature3 = video_feature + self.convtdvid2(tfidffea.unsqueeze(2)).squeeze(2)
        video_feature3 = self.convup3(video_feature3.unsqueeze(2)).squeeze(2)
        video_feature = torch.cat((video_feature, tfidffea), dim=1) + video_feature3
        video_feature = self.trans0(video_feature)
        features1 = Text_features + self.convvidtxt2(video_feature.unsqueeze(2)).squeeze(2)
        features1 = self.convup4(features1.unsqueeze(2)).squeeze(2)
        features = torch.cat((Text_features, video_feature), dim=1)  + features1
...
class VTBertForMaskedLM(BertPreTrainedModel):
    ...
    def forward(self, video_feature, video_feature1, text_input_ids, text_mask, is_usemask=False, return_mlm=False):
        encoder_outputs = self.bert(video_feature, video_feature1, text_input_ids, text_mask, is_usemask=is_usemask)
        if return_mlm:
...
class VTBert(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.convup = nn.Conv2d(in_channels=196, out_channels=196+32+1, kernel_size=(1, 1), stride=1, dilation=1, bias=False)
    ...
    # Copied from transformers.models.bert.modeling_bert.BertModel.forward
    def forward(self, video_feature, video_feature1, text_input_ids, text_mask, is_usemask=False):        
        ...
        video_feature1 += text_emb
        video_feature1 = self.convup(video_feature1.unsqueeze(2)).squeeze(2)
              

        # [CLS] Video [SEP] Text [SEP]
        embedding_output = torch.cat([text_emb, video_feature], 1)
        embedding_output += video_feature1
